...
const USER_ROLES=['admin','agent','broker','analyst'];
...
async function initializeAuthUsers(){
  ...
  memoryUsers=await Promise.all(DEMO_USERS.map(async({password,...user})=>({...user,passwordHash:await bcrypt.hash(password,12),active:true,mustChangePassword:false,sessionVersion:1,createdAt,updatedAt:createdAt})));
}
function initializeMemoryPolicies(){
  const timestamp=new Date().toISOString();
  memoryClients.forEach(client=>{if(client.policyNumber){client.policyVersion=Number(client.policyVersion||1);client.policyVersions=client.policyVersions||[];return}const basePremium=client.insuranceType==='Putno'?3500:client.insuranceType==='Životno'?12000:client.insuranceType==='Auto'?18000:9000;Object.assign(client,{policyNumber:`LEGACY-${client.id.toUpperCase()}`,policyVersion:1,policyVersions:[],validFrom:client.saleDate,validUntil:`${Number(client.saleDate.slice(0,4))+1}${client.saleDate.slice(4)}`,premium:basePremium+client.age*25,currency:'RSD',policyStatus:'Aktivna',paymentMethod:'Nije evidentirano',paymentStatus:'Plaćeno',insuredSubject:`${client.name} ${client.surname}`,documents:[],soldBy:{id:'system',username:'system',displayName:'Demo data migration'},policyHistory:[{action:'created',timestamp,performedBy:{id:'system',username:'system',displayName:'Demo data migration'}}]})});
}
async function init(){
  ...
  try{
    ...
    await db.collection('users').ensureIndex({type:'persistent',name:'idx_user_username',fields:['username'],unique:true,sparse:false});
    await db.collection('users').ensureIndex({type:'persistent',name:'idx_tenant_user_role',fields:['tenantId','role'],unique:false,sparse:false});
    ...
    await db.query(aql`
      FOR client IN clients
        ...
        UPDATE client WITH {
          policyNumber:CONCAT("LEGACY-",UPPER(client.id)),policyVersion:1,policyVersions:[],validFrom:client.saleDate,validUntil:SUBSTRING(DATE_ADD(client.saleDate,1,"year"),0,10),
    ...
    `);
    await db.query(aql`FOR client IN clients FILTER !HAS(client,"policyVersion") UPDATE client WITH {policyVersion:1,policyVersions:[]} IN clients`);
    ...
    for(const user of memoryUsers){
      ...
      await db.query(aql`UPSERT {_key:${user.id}} INSERT ${userDocument} UPDATE {} IN users`);
    }
    await db.query(aql`FOR user IN users FILTER !HAS(user,"sessionVersion") UPDATE user WITH {sessionVersion:1,mustChangePassword:false,updatedAt:${migrationTimestamp}} IN users`);
    mode='arango';
...
function insuranceKpis(clients,payments,claims){
  const currentPolicyKeys=new Set(clients.map(item=>`${item.id}:${item.policyNumber}`));payments=payments.filter(item=>currentPolicyKeys.has(`${item.clientId}:${item.policyNumber}`));claims=claims.filter(item=>currentPolicyKeys.has(`${item.clientId}:${item.policyNumber}`));
  ...
  const collectedFor=client=>{const items=(paymentsByClient.get(client.id)||[]).filter(item=>item.policyNumber===client.policyNumber),recorded=items.filter(item=>item.currency===client.currency).reduce((sum,item)=>sum+Number(item.amount||0),0);return items.length?Math.min(Number(client.premium||0),recorded):client.paymentStatus==='Plaćeno'?Number(client.premium||0):0};
...
async function renewPolicy(clientId,tenantId,user,input){
  const clients=await all('clients',memoryClients,tenantId),client=clients.find(item=>item.id===clientId);if(!client)return null;
  const timestamp=new Date().toISOString(),performedBy={id:user.id,username:user.username,displayName:user.displayName},newPolicyNumber=newPolicyNumberForRenewal(tenantId),previousVersion=Number(client.policyVersion||1),nextVersion=previousVersion+1,archived={version:previousVersion,policyNumber:client.policyNumber,insuranceType:client.insuranceType,insurer:client.insurer,saleDate:client.saleDate,validFrom:client.validFrom,validUntil:client.validUntil,premium:client.premium,currency:client.currency,policyStatus:client.policyStatus,paymentMethod:client.paymentMethod,paymentStatus:client.paymentStatus,insuredSubject:client.insuredSubject,destination:client.destination,passportNumber:client.passportNumber,vehicleMake:client.vehicleMake,engineCapacity:client.engineCapacity,vehicleType:client.vehicleType,bodyType:client.bodyType,brokerApproval:client.brokerApproval,soldBy:client.soldBy,documents:client.documents||[],archivedAt:timestamp,replacedByPolicyNumber:newPolicyNumber},historyEntry={action:'policy_renewed',timestamp,performedBy,previousPolicyNumber:client.policyNumber,policyNumber:newPolicyNumber,previousVersion,version:nextVersion},update={policyVersions:[...(client.policyVersions||[]),archived],policyVersion:nextVersion,renewedFromPolicyNumber:client.policyNumber,policyNumber:newPolicyNumber,saleDate:input.saleDate,validFrom:input.validFrom,validUntil:input.validUntil,premium:input.premium,currency:input.currency,insurer:input.insurer,paymentMethod:input.paymentMethod,paymentStatus:'Neplaćeno',policyStatus:'Aktivna',insuredSubject:input.insuredSubject,documents:[],soldBy:performedBy,policyHistory:[...(client.policyHistory||[]),historyEntry]};if(client.insuranceType==='Auto')update.brokerApproval={status:'pending',confirmedAt:null,confirmedBy:null};
  if(mode!=='arango'){const stored=memoryClients.find(item=>item.id===clientId&&item.tenantId===tenantId);Object.assign(stored,update);return stored}
  const cursor=await db.query(aql`FOR stored IN clients FILTER stored.id==${clientId} AND stored.tenantId==${tenantId} AND stored.policyNumber==${client.policyNumber} LIMIT 1 UPDATE stored WITH ${update} IN clients RETURN NEW`),renewed=await cursor.next();return renewed?clean(renewed):false;
}
function newPolicyNumberForRenewal(tenantId){return newPolicyNumber(tenantId)}
...
function publicUser(user){return{id:user.id,tenantId:user.tenantId,tenantName:user.tenantName,username:user.username,displayName:user.displayName,role:user.role,active:user.active!==false,mustChangePassword:Boolean(user.mustChangePassword),demoReadOnly:PUBLIC_DEMO&&user.role==='analyst'}}
function managedUser(user){return{id:user.id,username:user.username,displayName:user.displayName,role:user.role,active:user.active!==false,mustChangePassword:Boolean(user.mustChangePassword),createdAt:user.createdAt||null,updatedAt:user.updatedAt||null,lastLoginAt:user.lastLoginAt||null}}
...
async function findUserById(userId,tenantId){
  if(mode!=='arango')return memoryUsers.find(user=>user.id===userId&&user.tenantId===tenantId)||null;
  const cursor=await db.query(aql`FOR user IN users FILTER user._key==${userId} AND user.tenantId==${tenantId} LIMIT 1 RETURN user`);return(await cursor.next())||null;
}
async function updateManagedUser(userId,tenantId,changes){
  const updatedAt=new Date().toISOString(),update={...changes,updatedAt};
  if(mode!=='arango'){const user=memoryUsers.find(item=>item.id===userId&&item.tenantId===tenantId);if(!user)return null;Object.assign(user,update);return user}
  const cursor=await db.query(aql`FOR user IN users FILTER user._key==${userId} AND user.tenantId==${tenantId} LIMIT 1 UPDATE user WITH ${update} IN users RETURN NEW`);return(await cursor.next())||null;
}
...
async function authenticate(req,res,next){
  ...
  try{const claims=jwt.verify(token,JWT_SECRET),user=await findUserById(claims.id,claims.tenantId);if(!user||user.active===false||Number(claims.sessionVersion||1)!==Number(user.sessionVersion||1))throw new Error('revoked');req.user=publicUser(user);next()}catch(error){return res.status(401).json({message:'Your session has expired. Please sign in again.'})}
}
...
app.post('/api/auth/login',loginLimiter,async(req,res,next)=>{try{
  ...
  await recordLoginAttempt(req,username,true,'success',user.tenantId);user.lastLoginAt=new Date().toISOString();if(mode==='arango')await updateManagedUser(user.id,user.tenantId,{lastLoginAt:user.lastLoginAt});
  const sessionUser=publicUser(user),token=jwt.sign({...sessionUser,sessionVersion:user.sessionVersion||1},JWT_SECRET,{expiresIn:TOKEN_TTL_SECONDS,issuer:'kotva'});
...
app.post('/api/auth/change-password',authenticate,async(req,res,next)=>{try{const currentPassword=String(req.body.currentPassword||''),newPassword=String(req.body.newPassword||''),user=await findUserById(req.user.id,req.user.tenantId);if(!await bcrypt.compare(currentPassword,user.passwordHash))return res.status(400).json({message:'Current password is incorrect.'});if(newPassword.length<12||newPassword.length>128)return res.status(400).json({message:'New password must contain between 12 and 128 characters.'});if(await bcrypt.compare(newPassword,user.passwordHash))return res.status(400).json({message:'New password must be different from the current password.'});const updated=await updateManagedUser(user.id,user.tenantId,{passwordHash:await bcrypt.hash(newPassword,12),mustChangePassword:false,sessionVersion:Number(user.sessionVersion||1)+1}),sessionUser=publicUser(updated),token=jwt.sign({...sessionUser,sessionVersion:updated.sessionVersion},JWT_SECRET,{expiresIn:TOKEN_TTL_SECONDS,issuer:'kotva'});res.cookie('kotva_session',token,{httpOnly:true,sameSite:'strict',secure:process.env.NODE_ENV==='production',maxAge:TOKEN_TTL_SECONDS*1000,path:'/'});res.json({user:sessionUser})}catch(error){next(error)}});
app.use('/api',authenticate);
app.use('/api',(req,res,next)=>req.user.mustChangePassword?res.status(403).json({message:'You must change your temporary password before continuing.'}):next());
...
app.get('/api/users',authorize('admin'),async(req,res,next)=>{try{const users=mode==='arango'?(await (await db.query(aql`FOR user IN users FILTER user.tenantId==${req.user.tenantId} SORT user.displayName RETURN user`)).all()):memoryUsers.filter(user=>user.tenantId===req.user.tenantId).sort((a,b)=>a.displayName.localeCompare(b.displayName));res.json(users.map(managedUser))}catch(error){next(error)}});
app.post('/api/users',authorize('admin'),async(req,res,next)=>{try{const username=String(req.body.username||'').trim().toLowerCase(),displayName=String(req.body.displayName||'').trim(),role=String(req.body.role||''),suppliedPassword=String(req.body.temporaryPassword||'');if(!/^[a-z0-9._-]{3,40}$/.test(username))return res.status(400).json({message:'Username must contain 3-40 lowercase letters, numbers, dots, underscores or hyphens.'});if(displayName.length<2||displayName.length>80)return res.status(400).json({message:'Display name must contain between 2 and 80 characters.'});if(!USER_ROLES.includes(role))return res.status(400).json({message:'Select a supported role.'});if(await findUser(username))return res.status(409).json({message:'That username already exists.'});if(suppliedPassword&&(suppliedPassword.length<12||suppliedPassword.length>128))return res.status(400).json({message:'Temporary password must contain between 12 and 128 characters.'});const temporaryPassword=suppliedPassword||`Kotva-${crypto.randomBytes(9).toString('base64url')}`,timestamp=new Date().toISOString(),record={id:id('user'),tenantId:req.user.tenantId,tenantName:req.user.tenantName,username,displayName,role,passwordHash:await bcrypt.hash(temporaryPassword,12),active:true,mustChangePassword:true,sessionVersion:1,createdAt:timestamp,updatedAt:timestamp,lastLoginAt:null};if(mode==='arango')await db.collection('users').save({...record,_key:record.id});else memoryUsers.push(record);await recordBusinessAudit(req,'user.created','user',record.id,`Kreiran korisnički nalog ${username}`,{username,displayName,role});res.status(201).json({user:managedUser(record),temporaryPassword})}catch(error){next(error)}});
app.patch('/api/users/:id/status',authorize('admin'),async(req,res,next)=>{try{const active=req.body?.active;if(typeof active!=='boolean')return res.status(400).json({message:'Active status must be a boolean.'});if(req.params.id===req.user.id&&!active)return res.status(400).json({message:'You cannot deactivate your own account.'});const current=await findUserById(String(req.params.id),req.user.tenantId);if(!current)return res.status(404).json({message:'User was not found.'});const updated=await updateManagedUser(current.id,current.tenantId,{active,sessionVersion:Number(current.sessionVersion||1)+1});await recordBusinessAudit(req,active?'user.activated':'user.deactivated','user',updated.id,`${active?'Aktiviran':'Deaktiviran'} korisnički nalog ${updated.username}`,{active});res.json({user:managedUser(updated)})}catch(error){next(error)}});
app.patch('/api/users/:id/role',authorize('admin'),async(req,res,next)=>{try{const role=String(req.body?.role||'');if(!USER_ROLES.includes(role))return res.status(400).json({message:'Select a supported role.'});if(req.params.id===req.user.id&&role!=='admin')return res.status(400).json({message:'You cannot remove your own administrator role.'});const current=await findUserById(String(req.params.id),req.user.tenantId);if(!current)return res.status(404).json({message:'User was not found.'});const previousRole=current.role,updated=await updateManagedUser(current.id,current.tenantId,{role,sessionVersion:Number(current.sessionVersion||1)+1});await recordBusinessAudit(req,'user.role_updated','user',updated.id,`Promenjena uloga naloga ${updated.username} na ${role}`,{previousRole,role});res.json({user:managedUser(updated)})}catch(error){next(error)}});
app.post('/api/users/:id/reset-password',authorize('admin'),async(req,res,next)=>{try{const current=await findUserById(String(req.params.id),req.user.tenantId);if(!current)return res.status(404).json({message:'User was not found.'});const supplied=String(req.body?.temporaryPassword||'');if(supplied&&(supplied.length<12||supplied.length>128))return res.status(400).json({message:'Temporary password must contain between 12 and 128 characters.'});const temporaryPassword=supplied||`Kotva-${crypto.randomBytes(9).toString('base64url')}`,updated=await updateManagedUser(current.id,current.tenantId,{passwordHash:await bcrypt.hash(temporaryPassword,12),mustChangePassword:true,sessionVersion:Number(current.sessionVersion||1)+1});await recordBusinessAudit(req,'user.password_reset','user',updated.id,`Resetovana lozinka naloga ${updated.username}`,{});res.json({user:managedUser(updated),temporaryPassword})}catch(error){next(error)}});
...
app.get('/api/config',(q,r)=>r.json({insuranceTypes:TYPES,vehicleTypes:VEHICLE_TYPES,bodyTypes:BODY_TYPES,policyStatuses:POLICY_STATUSES,paymentMethods:PAYMENT_METHODS,paymentStatuses:PAYMENT_STATUSES,currencies:CURRENCIES,claimStatuses:CLAIM_STATUSES,tenant:{id:q.user.tenantId,name:q.user.tenantName}}));app.get('/api/clients',async(q,r,n)=>{try{
  const wantsPage=['page','pageSize','q','insuranceType','insurer','policyStatus','paymentStatus','dateFrom','dateTo','sortBy','order'].some(key=>q.query[key]!==undefined);if(!wantsPage)return r.json(await all('clients',memoryClients,q.user.tenantId));
  const page=Number(q.query.page||1),pageSize=Number(q.query.pageSize||10),sortBy=String(q.query.sortBy||'createdAt'),order=String(q.query.order||'desc').toLowerCase(),allowedSort=new Set(['createdAt','saleDate','name','premium','validUntil','policyNumber']);if(!Number.isInteger(page)||page<1||!Number.isInteger(pageSize)||pageSize<1||pageSize>50||!allowedSort.has(sortBy)||!['asc','desc'].includes(order))return r.status(400).json({message:'Parametri paginacije ili sortiranja nisu ispravni.'});
  const fold=value=>String(value||'').normalize('NFD').replace(/\p{Diacritic}/gu,'').toLocaleLowerCase('sr'),term=fold(q.query.q).trim(),insuranceType=String(q.query.insuranceType||''),insurer=String(q.query.insurer||''),policyStatus=String(q.query.policyStatus||''),paymentStatus=String(q.query.paymentStatus||''),dateFrom=String(q.query.dateFrom||''),dateTo=String(q.query.dateTo||'');let records=await all('clients',memoryClients,q.user.tenantId);records=records.filter(item=>(!term||fold(`${item.name} ${item.surname} ${item.policyNumber||''}`).includes(term))&&(!insuranceType||item.insuranceType===insuranceType)&&(!insurer||item.insurer===insurer)&&(!policyStatus||item.policyStatus===policyStatus)&&(!paymentStatus||item.paymentStatus===paymentStatus)&&(!dateFrom||item.saleDate>=dateFrom)&&(!dateTo||item.saleDate<=dateTo));const direction=order==='asc'?1:-1,sortValue=item=>sortBy==='name'?fold(`${item.surname} ${item.name}`):sortBy==='premium'?Number(item.premium||0):sortBy==='createdAt'?String(item.createdAt||item.policyHistory?.at(-1)?.timestamp||''):String(item[sortBy]||'');records.sort((left,right)=>{const a=sortValue(left),b=sortValue(right);return(a>b?1:a<b?-1:0)*direction});const totalItems=records.length,totalPages=Math.max(1,Math.ceil(totalItems/pageSize)),safePage=Math.min(page,totalPages),offset=(safePage-1)*pageSize;
  r.json({items:records.slice(offset,offset+pageSize),pagination:{page:safePage,pageSize,totalItems,totalPages,hasPrevious:safePage>1,hasNext:safePage<totalPages},appliedFilters:{q:term,insuranceType,insurer,policyStatus,paymentStatus,dateFrom,dateTo,sortBy,order}})
}catch(e){n(e)}});app.get('/api/insurers',async(q,r,n)=>{try{r.json(await all('insurers',memoryInsurers,q.user.tenantId))}catch(e){n(e)}});app.get('/api/claims',async(q,r,n)=>{try{const claims=await all('claims',memoryClaims,q.user.tenantId);r.json(claims.sort((a,b)=>b.reportedAt.localeCompare(a.reportedAt)))}catch(e){n(e)}});app.get('/api/payments',async(q,r,n)=>{try{const payments=await all('payments',memoryPayments,q.user.tenantId);r.json(payments.sort((a,b)=>b.recordedAt.localeCompare(a.recordedAt)))}catch(e){n(e)}});
...
app.post('/api/clients',authorize('admin','agent','broker'),async(q,r,n)=>{try{
  ...
  x.policyNumber=newPolicyNumber(q.user.tenantId);x.createdAt=timestamp;x.policyVersion=1;x.policyVersions=[];x.soldBy=performedBy;x.documents=documentReference?[{id:id('document'),name:'Prateći dokument',reference:documentReference,addedAt:timestamp,addedBy:performedBy}]:[];x.policyHistory=[{action:'created',timestamp,performedBy,status:x.policyStatus,paymentStatus:x.paymentStatus}];
...
app.post('/api/clients/:id/broker-approval',authorize('admin','agent','broker'),async(q,r,n)=>{try{
...
app.patch('/api/clients/:id/policy',authorize('admin','agent','broker'),async(q,r,n)=>{try{
...
app.post('/api/clients/:id/renew',authorize('admin','agent','broker'),async(q,r,n)=>{try{
  const clientId=String(q.params.id||''),current=(await all('clients',memoryClients,q.user.tenantId)).find(item=>item.id===clientId);if(!current)return r.status(404).json({message:'Polisa nije pronađena.'});const previousPolicyNumber=current.policyNumber,saleDate=String(q.body?.saleDate||''),validFrom=String(q.body?.validFrom||''),validUntil=String(q.body?.validUntil||''),premium=Number(q.body?.premium),currency=String(q.body?.currency||current.currency||''),insurer=String(q.body?.insurer||current.insurer||'').trim(),paymentMethod=String(q.body?.paymentMethod||current.paymentMethod||''),insuredSubject=String(q.body?.insuredSubject||current.insuredSubject||'').trim();
  if(!validIsoDate(saleDate)||!validIsoDate(validFrom)||!validIsoDate(validUntil))return r.status(400).json({message:'Datumi obnove nisu ispravni.'});if(saleDate>validFrom)return r.status(400).json({message:'Datum prodaje obnove ne može biti posle početka važenja.'});if(validFrom<=current.validUntil)return r.status(400).json({message:'Nova verzija polise mora početi nakon isteka trenutne verzije.'});if(validUntil<=validFrom)return r.status(400).json({message:'Datum isteka mora biti posle početka važenja.'});if(!Number.isFinite(premium)||premium<=0||premium>100000000)return r.status(400).json({message:'Premija mora biti pozitivan iznos do 100.000.000.'});if(!CURRENCIES.includes(currency))return r.status(400).json({message:'Izaberite podržanu valutu.'});if(!PAYMENT_METHODS.includes(paymentMethod))return r.status(400).json({message:'Izaberite način plaćanja.'});if(insuredSubject.length<2||insuredSubject.length>200)return r.status(400).json({message:'Predmet osiguranja mora imati od 2 do 200 znakova.'});if(!(await all('insurers',memoryInsurers,q.user.tenantId)).some(item=>item.name===insurer))return r.status(400).json({message:'Izaberite postojeću osiguravajuću kuću.'});
  const renewed=await renewPolicy(clientId,q.user.tenantId,q.user,{saleDate,validFrom,validUntil,premium,currency,insurer,paymentMethod,insuredSubject});if(!renewed)return r.status(409).json({message:'Polisa je u međuvremenu promenjena. Osvežite stranicu i pokušajte ponovo.'});await syncInsuranceGraph();await recordBusinessAudit(q,'policy.renewed','policy',renewed.id,`Obnovljena polisa ${previousPolicyNumber} kao ${renewed.policyNumber}`,{previousPolicyNumber,policyNumber:renewed.policyNumber,version:renewed.policyVersion,validFrom,validUntil,premium,currency});r.status(201).json(renewed);
}catch(e){n(e)}});
app.post('/api/claims',authorize('admin','agent','broker'),async(q,r,n)=>{try{
...
app.patch('/api/claims/:id/status',authorize('admin','agent','broker'),async(q,r,n)=>{try{const claimId=String(q.params.id||''),status=String(q.body.status||'');if(!claimId||claimId.length>120)return r.status(400).json({message:'Identifikator štete nije ispravan.'});if(!CLAIM_STATUSES.includes(status))return r.status(400).json({message:'Izaberite važeći status štete.'});const claim=await updateClaimStatus(claimId,q.user.tenantId,q.user,status);if(!claim)return r.status(404).json({message:'Šteta nije pronađena.'});await recordBusinessAudit(q,'claim.status_updated','claim',claim.id,`Status štete ${claim.claimNumber} promenjen na ${status}`,{status});r.json(claim)}catch(e){n(e)}});
app.post('/api/payments',authorize('admin','agent','broker'),async(q,r,n)=>{try{
...
app.get('/api/clients/:id/details',async(q,r,n)=>{try{
  const clientId=String(q.params.id||''),[clients,claims,payments,documents]=await Promise.all([all('clients',memoryClients,q.user.tenantId),all('claims',memoryClaims,q.user.tenantId),all('payments',memoryPayments,q.user.tenantId),all('policy_documents',memoryPolicyDocuments,q.user.tenantId)]),client=clients.find(item=>item.id===clientId);if(!client)return r.status(404).json({message:'Polisa nije pronađena.'});
  const relatedClaims=claims.filter(item=>item.clientId===client.id).sort((a,b)=>b.reportedAt.localeCompare(a.reportedAt)),relatedPayments=payments.filter(item=>item.clientId===client.id).sort((a,b)=>b.recordedAt.localeCompare(a.recordedAt)),relatedDocuments=documents.filter(item=>item.clientId===client.id).sort((a,b)=>b.uploadedAt.localeCompare(a.uploadedAt)).map(({storageName,...document})=>clean(document)),currentClaims=relatedClaims.filter(item=>item.policyNumber===client.policyNumber),currentPayments=relatedPayments.filter(item=>item.policyNumber===client.policyNumber),currentDocuments=relatedDocuments.filter(item=>item.policyNumber===client.policyNumber),recordedPaid=+currentPayments.reduce((sum,item)=>sum+Number(item.amount||0),0).toFixed(2),totalPaid=currentPayments.length?recordedPaid:client.paymentStatus==='Plaćeno'?Number(client.premium||0):recordedPaid,outstanding=Math.max(0,+(Number(client.premium||0)-totalPaid).toFixed(2));
  const timeline=[...(client.policyHistory||[]).filter(item=>item.action!=='payment_recorded').map(item=>({type:'policy',action:item.action,timestamp:item.timestamp,performedBy:item.performedBy,reference:item.action==='policy_renewed'?`${item.previousPolicyNumber} → ${item.policyNumber}`:null,details:{status:item.status,paymentStatus:item.paymentStatus,changes:item.changes,previousVersion:item.previousVersion,version:item.version}})),...relatedClaims.flatMap(claim=>(claim.history||[]).map(item=>({type:'claim',action:item.action,timestamp:item.timestamp,performedBy:item.performedBy,reference:claim.claimNumber,details:{status:item.status,previousStatus:item.previousStatus}}))),...relatedDocuments.map(document=>({type:'document',action:'document_uploaded',timestamp:document.uploadedAt,performedBy:document.uploadedBy,reference:document.originalName})),...relatedPayments.map(payment=>({type:'payment',action:'payment_recorded',timestamp:payment.recordedAt,performedBy:payment.recordedBy,reference:payment.receiptNumber,details:{amount:payment.amount,currency:payment.currency}}))].filter(item=>item.timestamp).sort((a,b)=>b.timestamp.localeCompare(a.timestamp));
  const policy=clean({...client});if(q.user.role==='analyst'){if(policy.jmbg)policy.jmbg=`*********${policy.jmbg.slice(-4)}`;if(policy.passportNumber)policy.passportNumber=`****${policy.passportNumber.slice(-3)}`;policy.policyVersions=(policy.policyVersions||[]).map(version=>({...version,passportNumber:version.passportNumber?`****${version.passportNumber.slice(-3)}`:version.passportNumber}))}
  r.json({policy,claims:relatedClaims,payments:relatedPayments,documents:relatedDocuments,timeline,summary:{totalPaid,outstanding,claimCount:currentClaims.length,estimatedClaimAmount:+currentClaims.reduce((sum,item)=>sum+Number(item.estimatedAmount||0),0).toFixed(2),documentCount:currentDocuments.length,versionCount:Number(client.policyVersion||1)},permissions:{canWrite:['admin','agent','broker'].includes(q.user.role),canManageInsurer:q.user.role==='admin'}});
}catch(e){n(e)}});
