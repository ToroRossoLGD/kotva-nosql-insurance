const express=require('express');const path=require('path');const cors=require('cors');const{Database,aql}=require('arangojs');require('dotenv').config();
...
let memoryInsurers=[...INSURERS],memoryClients=[...CLIENTS],db,mode='memory';const clean=({_key,_id,_rev,...x})=>x,id=p=>`${p}-${Date.now()}-${Math.random().toString(16).slice(2,8)}`;
async function init(){
  ...
  try{
    ...
    for(const name of['clients','insurers']){const col=db.collection(name);if(!(await col.exists()))await col.create()}
...
app.get('/api/health',(q,r)=>r.json({status:'ok',database:mode,databaseName:DB_NAME}));app.get('/api/config',(q,r)=>r.json({insuranceTypes:TYPES}));app.get('/api/clients',async(q,r,n)=>{try{r.json(await all('clients',memoryClients))}catch(e){n(e)}});app.get('/api/insurers',async(q,r,n)=>{try{r.json(await all('insurers',memoryInsurers))}catch(e){n(e)}});
app.post('/api/insurers',async(q,r,n)=>{try{const name=String(q.body.name||'').trim();if(name.length<2||name.length>80)return r.status(400).json({message:'Naziv mora imati od 2 do 80 znakova.'});const current=await all('insurers',memoryInsurers);if(current.some(x=>x.name.localeCompare(name,'sr',{sensitivity:'base'})===0))return r.status(409).json({message:'Ta osiguravajuća kuća već postoji.'});r.status(201).json(await save('insurers',{name},memoryInsurers))}catch(e){n(e)}});
app.post('/api/clients',async(q,r,n)=>{try{
