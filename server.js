...
async function init(){
  ...
  try{
    ...
    for(const name of['clients','insurers','users','login_attempts','tenants','claims','payments','notification_dismissals']){const col=db.collection(name);if(!(await col.exists()))await col.create()}
    ...
    else{const edges=await graph.listEdgeCollections();if(!edges.includes('has_claim'))await graph.addEdgeDefinition({collection:'has_claim',from:['policies'],to:['claims']});if(!edges.includes('has_payment'))await graph.addEdgeDefinition({collection:'has_payment',from:['policies'],to:['payments']})}
    for(const collectionName of['clients','insurers','policies','claims','payments','owns','issued_by','has_claim','has_payment']){
