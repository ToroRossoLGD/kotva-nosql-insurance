...
async function init(){
  ...
  try{
    ...
    for(const name of['clients','insurers','users','login_attempts','business_audit','tenants','claims','payments','policy_documents','notification_dismissals']){const col=db.collection(name);if(!(await col.exists()))await col.create()}
