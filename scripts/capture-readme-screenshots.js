...
async function main(){
  ...
  await page.goto(baseUrl);await page.locator('#login-form [name="username"]').fill('analyst');await page.locator('#login-form [name="password"]').fill('Analyst123!');await page.locator('#login-form button').click();await page.locator('#app-shell').waitFor({state:'visible'});await page.locator('#clients-body .policy-detail-link').first().waitFor();
