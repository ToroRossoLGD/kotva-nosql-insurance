...
test('quote workflow enforces status transitions and converts an accepted quote once',async()=>{
  const agent=await login('agent','Agent123!'),analyst=await login('analyst','Analyst123!'),payload={name:'Quote',surname:'Customer',age:37,insuranceType:'DZO',insurer:'Uniqa',validFrom:'2026-10-01',validUntil:'2027-10-01',premium:14500,currency:'RSD',insuredSubject:'Quote Customer'};const forbidden=await request('/api/quotes',{method:'POST',headers:{cookie:analyst.cookie,'content-type':'application/json'},body:JSON.stringify(payload)});assert.equal(forbidden.response.status,403);const created=await request('/api/quotes',{method:'POST',headers:{cookie:agent.cookie,'content-type':'application/json'},body:JSON.stringify(payload)});assert.equal(created.response.status,201);assert.match(created.body.quoteNumber,/^QTE-/);assert.equal(created.body.status,'Nacrt');const invalid=await request(`/api/quotes/${created.body.id}/status`,{method:'PATCH',headers:{cookie:agent.cookie,'content-type':'application/json'},body:JSON.stringify({status:'Prihvaćena'})});assert.equal(invalid.response.status,409);for(const status of['Poslata','Prihvaćena']){const changed=await request(`/api/quotes/${created.body.id}/status`,{method:'PATCH',headers:{cookie:agent.cookie,'content-type':'application/json'},body:JSON.stringify({status})});assert.equal(changed.response.status,200);assert.equal(changed.body.status,status)}const converted=await request(`/api/quotes/${created.body.id}/convert`,{method:'POST',headers:{cookie:agent.cookie}});assert.equal(converted.response.status,201);assert.match(converted.body.policy.policyNumber,/^POL-/);assert.equal(converted.body.policy.sourceQuoteNumber,created.body.quoteNumber);assert.equal(converted.body.quote.status,'Konvertovana');assert.equal((await request(`/api/quotes/${created.body.id}/convert`,{method:'POST',headers:{cookie:agent.cookie}})).response.status,409);const quotes=await request('/api/quotes',{headers:{cookie:analyst.cookie}});assert.ok(quotes.body.some(item=>item.id===created.body.id&&item.convertedPolicyNumber===converted.body.policy.policyNumber));
});

test('pricing engine explains premiums and versions tenant rating rules',async()=>{
  const agent=await login('agent','Agent123!'),admin=await login('admin','Admin123!'),analyst=await login('analyst','Analyst123!'),input={insuranceType:'Auto',age:22,validFrom:'2026-10-01',validUntil:'2027-10-01',engineCapacity:2200,vehicleType:'Putničko',bodyType:'SUV'};const calculation=await request('/api/rating/calculate',{method:'POST',headers:{cookie:agent.cookie,'content-type':'application/json'},body:JSON.stringify(input)});assert.equal(calculation.response.status,200);assert.equal(calculation.body.basePremium,15000);assert.ok(calculation.body.recommendedPremium>calculation.body.basePremium);assert.ok(calculation.body.adjustments.some(item=>item.code==='young_age'));assert.ok(calculation.body.adjustments.some(item=>item.code==='engine_capacity'));assert.ok(calculation.body.adjustments.some(item=>item.code==='vehicle_type'));assert.equal((await request('/api/rating-rules/Auto',{method:'PATCH',headers:{cookie:agent.cookie,'content-type':'application/json'},body:'{}'})).response.status,403);const updated=await request('/api/rating-rules/DZO',{method:'PATCH',headers:{cookie:admin.cookie,'content-type':'application/json'},body:JSON.stringify({basePremium:7000,minPremium:5500,youngMultiplier:1.2,seniorMultiplier:1.3})});assert.equal(updated.response.status,200);assert.equal(updated.body.version,2);const rules=await request('/api/rating-rules',{headers:{cookie:analyst.cookie}});assert.equal(rules.response.status,200);assert.equal(rules.body.find(item=>item.insuranceType==='DZO').basePremium,7000);
});

test('CSV migration previews row errors and imports valid policies idempotently',async()=>{
  const agent=await login('agent','Agent123!'),broker=await login('broker','Broker123!'),headers=['externalId','name','surname','age','insuranceType','insurer','saleDate','validFrom','validUntil','premium','currency','policyStatus','paymentMethod','paymentStatus','insuredSubject','jmbg','passportNumber','destination','vehicleMake','engineCapacity','vehicleType','bodyType'],csvRow=values=>values.map(value=>`"${String(value??'').replaceAll('"','""')}"`).join(','),valid=csvRow(headers)+'\n'+csvRow(['IMPORT-001','Import','Valid','38','DZO','Uniqa','2026-09-16','2026-09-16','2027-09-16','11000','RSD','Aktivna','Kartica','Neplaćeno','Import Valid']),invalid=csvRow(headers)+'\n'+csvRow(['IMPORT-BAD','Import','Invalid','999','Nepoznato','Nema','bad','bad','bad','-1','USD','X','X','X','']);assert.equal((await request('/api/imports/policies/preview',{method:'POST',headers:{cookie:broker.cookie,'content-type':'application/json'},body:JSON.stringify({csv:valid})})).response.status,403);const previewBad=await request('/api/imports/policies/preview',{method:'POST',headers:{cookie:agent.cookie,'content-type':'application/json'},body:JSON.stringify({csv:invalid})});assert.equal(previewBad.response.status,200);assert.equal(previewBad.body.canImport,false);assert.equal(previewBad.body.invalidRows,1);assert.ok(previewBad.body.rows[0].errors.length>=3);assert.equal((await request('/api/imports/policies/commit',{method:'POST',headers:{cookie:agent.cookie,'content-type':'application/json'},body:JSON.stringify({csv:invalid})})).response.status,422);const preview=await request('/api/imports/policies/preview',{method:'POST',headers:{cookie:agent.cookie,'content-type':'application/json'},body:JSON.stringify({csv:valid})});assert.equal(preview.body.canImport,true);const committed=await request('/api/imports/policies/commit',{method:'POST',headers:{cookie:agent.cookie,'content-type':'application/json'},body:JSON.stringify({csv:valid,fileName:'migration.csv'})});assert.equal(committed.response.status,201);assert.equal(committed.body.importedRows,1);const clients=await request('/api/clients',{headers:{cookie:agent.cookie}});assert.ok(clients.body.some(item=>item.externalId==='IMPORT-001'&&item.sourceImportId===committed.body.id));const duplicate=await request('/api/imports/policies/preview',{method:'POST',headers:{cookie:agent.cookie,'content-type':'application/json'},body:JSON.stringify({csv:valid})});assert.equal(duplicate.body.canImport,false);assert.match(duplicate.body.rows[0].errors.join(' '),/externalId/);const runs=await request('/api/imports/policies/runs',{headers:{cookie:agent.cookie}});assert.ok(runs.body.some(run=>run.id===committed.body.id));
});
...
test('policy detail endpoint returns a tenant-safe 360-degree operational view',async()=>{
  const agent=await login('agent','Agent123!'),policy=await request('/api/clients',{method:'POST',headers:{cookie:agent.cookie,'content-type':'application/json'},body:JSON.stringify(withPolicy({name:'Detail',surname:'Customer',age:37,insuranceType:'Putno',insurer:'Uniqa',saleDate:'2026-09-01'},{premium:2000,paymentStatus:'Neplaćeno',insuredSubject:'Putno osiguranje',jmbg:'0707990712345',passportNumber:'DETAIL123',destination:'Italija'}))});assert.equal(policy.response.status,201);
  const claim=await request('/api/claims',{method:'POST',headers:{cookie:agent.cookie,'content-type':'application/json'},body:JSON.stringify({clientId:policy.body.id,incidentDate:'2026-09-05',estimatedAmount:800,currency:'RSD',description:'Test štete na detaljnom prikazu polise.'})});assert.equal(claim.response.status,201);
  const payment=await request('/api/payments',{method:'POST',headers:{cookie:agent.cookie,'content-type':'application/json'},body:JSON.stringify({clientId:policy.body.id,paymentDate:'2026-09-06',amount:500,method:'Kartica',reference:'DETAIL-PAYMENT-001'})});assert.equal(payment.response.status,201);
  const form=new FormData();form.append('document',new Blob(['%PDF-1.4\nDetail page'],{type:'application/pdf'}),'detail-policy.pdf');const document=await request(`/api/clients/${policy.body.id}/documents`,{method:'POST',headers:{cookie:agent.cookie},body:form});assert.equal(document.response.status,201);
  const details=await request(`/api/clients/${policy.body.id}/details`,{headers:{cookie:agent.cookie}});assert.equal(details.response.status,200);assert.equal(details.body.policy.id,policy.body.id);assert.equal(details.body.claims.length,1);assert.equal(details.body.payments.length,1);assert.equal(details.body.documents.length,1);assert.equal(details.body.documents[0].storageName,undefined);assert.equal(details.body.summary.totalPaid,500);assert.equal(details.body.summary.outstanding,1500);assert.equal(details.body.summary.estimatedClaimAmount,800);assert.ok(details.body.timeline.some(item=>item.type==='claim'));assert.ok(details.body.timeline.some(item=>item.type==='payment'));assert.ok(details.body.timeline.some(item=>item.type==='document'));assert.equal(details.body.permissions.canWrite,true);
  const analyst=await login('analyst','Analyst123!'),analystDetails=await request(`/api/clients/${policy.body.id}/details`,{headers:{cookie:analyst.cookie}});assert.equal(analystDetails.response.status,200);assert.equal(analystDetails.body.permissions.canWrite,false);assert.equal(analystDetails.body.policy.jmbg,'*********2345');assert.equal(analystDetails.body.policy.passportNumber,'****123');assert.ok(!JSON.stringify(analystDetails.body).includes('0707990712345'));
  const adria=await login('adria-admin','Adria123!'),crossTenant=await request(`/api/clients/${policy.body.id}/details`,{headers:{cookie:adria.cookie}});assert.equal(crossTenant.response.status,404);
});

test('policy renewal archives immutable versions and starts clean current-policy balances',async()=>{
  const agent=await login('agent','Agent123!'),policy=await request('/api/clients',{method:'POST',headers:{cookie:agent.cookie,'content-type':'application/json'},body:JSON.stringify(withPolicy({name:'Renewal',surname:'Customer',age:45,insuranceType:'DZO',insurer:'Uniqa',saleDate:'2024-01-01'},{premium:1000,paymentStatus:'Neplaćeno',insuredSubject:'DZO obnova'}))});assert.equal(policy.response.status,201);assert.equal(policy.body.policyVersion,1);
