...
async function options(){const[i,c]=await Promise.all([api('/api/insurers'),api('/api/config')]);$('#insurer-select').innerHTML=i.map(x=>`<option>${escapeHtml(x.name)}</option>`).join('');$('#type-select').innerHTML=c.insuranceTypes.map(x=>`<option>${escapeHtml(x)}</option>`).join('');$('#vehicle-type-select').innerHTML=c.vehicleTypes.map(x=>`<option>${escapeHtml(x)}</option>`).join('');$('#body-type-select').innerHTML=c.bodyTypes.map(x=>`<option>${escapeHtml(x)}</option>`).join('');$('#currency-select').innerHTML=c.currencies.map(x=>`<option>${escapeHtml(x)}</option>`).join('');$('#policy-status-select').innerHTML=c.policyStatuses.map(x=>`<option>${escapeHtml(x)}</option>`).join('');$('#payment-method-select').innerHTML=c.paymentMethods.map(x=>`<option>${escapeHtml(x)}</option>`).join('');$('#payment-status-select').innerHTML=c.paymentStatuses.map(x=>`<option>${escapeHtml(x)}</option>`).join('');toggleConditionalFields()}
